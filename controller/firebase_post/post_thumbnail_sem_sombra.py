...
def upload_thumbnail_sem_sombra():
    """
    Faz o upload de um thumbnail sem sombra para o Firebase e retorna a URL pública.
    """
    # Verifica se o arquivo está presente na requisição
    if 'thumbnail_sem_sombra' not in request.files:
        return jsonify({"error": "O thumbnail 'thumbnail_sem_sombra' é necessário."}), 400

    thumbnail_image = request.files['thumbnail_sem_sombra']
    ...
    available_name = get_next_available_filename(thumbnail_image.filename, 'thumbnails/sem_sombra')
    
    thumbnail_blob = bucket.blob(f'thumbnails/sem_sombra/{available_name}')
    ...
    return jsonify({
        "thumbnail_sem_sombra_url": thumbnail_url,
    })
