...
def upload_thumbnail_sem_nuvem():
    """
    Faz o upload de um thumbnail sem nuvem para o Firebase e retorna a URL pública.
    """
    # Verifica se o arquivo está presente na requisição
    if 'thumbnail_sem_nuvem' not in request.files:
        return jsonify({"error": "O thumbnail 'thumbnail_sem_nuvem' é necessário."}), 400

    thumbnail_image = request.files['thumbnail_sem_nuvem']
    ...
    available_name = get_next_available_filename(thumbnail_image.filename, 'thumbnails/sem_nuvem')
    
    thumbnail_blob = bucket.blob(f'thumbnails/sem_nuvem/{available_name}')
    ...
    return jsonify({
        "thumbnail_sem_nuvem_url": thumbnail_url,
    })
