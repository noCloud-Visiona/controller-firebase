...
def upload_thumbnail_imagem_original():
    """
    Faz o upload de um thumbnail da imagem original para o Firebase e retorna a URL pública.
    """
    # Verifica se o arquivo está presente na requisição
    if 'thumbnail_imagem_original' not in request.files:
        return jsonify({"error": "O thumbnail 'thumbnail_imagem_original' é necessário."}), 400

    thumbnail_image = request.files['thumbnail_imagem_original']
    ...
    available_name = get_next_available_filename(thumbnail_image.filename, 'thumbnails/imagem_original')
    
    thumbnail_blob = bucket.blob(f'thumbnails/imagem_original/{available_name}')
    ...
    return jsonify({
        "thumbnail_imagem_original_url": thumbnail_url,
    })
