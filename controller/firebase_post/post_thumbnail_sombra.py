...
def upload_thumbnail_sombra():
    """
    Faz o upload de um thumbnail de sombra para o Firebase e retorna a URL pública.
    """
    # Verifica se o arquivo está presente na requisição
    if 'thumbnail_sombra' not in request.files:
        return jsonify({"error": "O thumbnail 'thumbnail_sombra' é necessário."}), 400

    thumbnail_image = request.files['thumbnail_sombra']
    ...
    available_name = get_next_available_filename(thumbnail_image.filename, 'thumbnails/sombra')
    
    thumbnail_blob = bucket.blob(f'thumbnails/sombra/{available_name}')
    ...
    return jsonify({
        "thumbnail_sombra_url": thumbnail_url,
    })
